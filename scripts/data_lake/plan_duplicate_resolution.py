#!/usr/bin/env python
"""Stable duplicate-resolution planning entrypoint.

The planner is read-only. It classifies duplicate-key groups into exact-copy,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plan duplicate-key resolution without mutating the data lake.")
    ...
    report = {
        ...
        "classification_method": {
            ...
            "hash_note": "Hashes use pandas row fingerprints for routing. Future write gates must materialize exact rows and verify pre/post-write content.",
        },
        "totals": totals,
...
def _totals(dataset_results: Sequence[dict[str, Any]]) -> dict[str, int]:
    totals = {
