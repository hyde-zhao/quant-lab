...
from scripts.legacy.cr.run_chapter5_anomalies import assert_chapter4_report_pass
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行因子稳健性验证")
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id or f"run-factor-robustness-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
