#!/usr/bin/env python
"""CR139 Gate B Batch 0 duplicate profile.

The profiler is read-only. It scans canonical parquet files and writes a JSON
decision-evidence report outside the protected lake root. It does not mutate
lake objects, catalog metadata, pointers, or physical partitions.
"""
    ...
    report = {
        ...
        "profile_scope": {
            "global": [
                ...
                "top metadata values among duplicate rows",
            ],
            "sample_limited": [
            ...
            "rationale": "Full-row exact duplicate profiling across all columns for all rows is intentionally not executed by default to avoid unreviewed memory spill; Batch 0 supplies enough key/metadata evidence to choose whether each dataset should use deterministic dedup, quarantine, or deeper per-dataset profiling before writes.",
        },
    ...
    for _, group in sample_rows.groupby(sample_key_series, sort=False):
        for column in target_columns:
            if column in key_columns:
                continue
            if column not in sample_rows.columns:
                continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build CR139 Gate B Batch 0 duplicate profile evidence.")
