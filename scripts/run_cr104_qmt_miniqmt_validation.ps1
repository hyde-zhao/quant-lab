...
function Invoke-Cr104Checker {
    ...
    & uv run --python 3.11 python scripts/check_cr099_redacted_evidence.py --evidence $EvidencePath --json
...
if ($Mode -eq "DryRunPreflight") {
    ...
    & uv run --python 3.11 python scripts/build_cr104_runtime_preflight_evidence.py `
        --run-id $RunId `
...
$CollectorArgs = @(
    "run", "--python", "3.11", "python", "scripts/collect_cr099_runner_readonly_smoke.py",
    "--env-file", $EnvFile,
    "--authorization-ref", $AuthorizationRef,
    "--run-id", $RunId,
