...
当前实现以 Python 模块/API 为主，并提供轻量 identity CLI 入口 `quant_lab`。命令示例统一使用 `uv run --python 3.11`；需要检查当前 package identity 时可运行 `uv run quant_lab --help` 或 `uv run python -m quant_lab.cli --help`。
...
Agent 协作边界也按该目录组织执行：当前会话的 Host Orchestrator 主进程负责编排 CR、检查点和人工门禁；功能 Agent 只在对应阶段按需参与。后续学习笔记任务在 `llm-wiki` 中处理，工具代码、配置、测试和用户文档任务在 `quant-lab` 中处理。
...
完整路线、阶段目标、切换条件和不做事项见 [docs/ROADMAP.md](docs/ROADMAP.md)。对应架构决策记录见 [process/ARCHITECTURE-DECISION.md](process/ARCHITECTURE-DECISION.md) 的 ADR-023。
...
当前已验证命令结果见 [process/VERIFICATION-REPORT.md](process/VERIFICATION-REPORT.md) 与 [process/TEST-STRATEGY.md](process/TEST-STRATEGY.md)。最近一次 F-004 回归记录显示定向日志测试、全量 pytest 和 compileall 均通过，详见 [process/handoffs/META-QA-REGRESSION-F004-LOGGING-2026-05-16.md](process/handoffs/META-QA-REGRESSION-F004-LOGGING-2026-05-16.md)。
...
本轮用户运维决策是使用本地 `.env` 管理 Tushare token 与外置数据湖路径。`.env` / `.env.*` 已由 [.gitignore](.gitignore) 忽略，不提交、不生成带真实值的样例文件。文档中的本地 `.env` 内容只能使用以下占位形式：
...
项目代码只读取环境变量；`pyproject.toml` / `uv.lock` 已落地 `tushare==1.4.29` 与 `jqdatasdk` dependency group。`.env` 文件只能由用户本机维护；Agent 不得读取、打印或代为检查 `.env`、token、账号或任何凭据。Tushare 正式命令入口通过 `uv run --env-file .env --group tushare --python 3.11 ...` 加载本机 `.env` 并启用该 group，该类命令仅供用户在明确授权的本地环境中手工执行：
...
CR-013 新证据写入 `reports/data_lake_readiness_2020_2024_cr013/`，路线图见 `docs/DATA-LAKE-FULL-HISTORY-BACKFILL-ROADMAP.md`。当前 provider fetch、lake write、credential read、legacy data read、old report overwrite 均为 0；任何真实补数、VWAP / 分钟数据接入或 allowed claim 解除，都必须另起 Story、CP5 和用户显式授权。
...
验证摘要见 [CR011-S08 CP7](process/checks/CP7-CR011-S08-factor-panel-audit-and-robust-validation-VERIFICATION-DONE.md)：S08 定向测试 `3 passed`，上游与实验回归 `29 passed`，fail-closed probe 为 PASS；CR-011 全量 Story 状态见 [process/STORY-STATUS.md](process/STORY-STATUS.md)。

### CR-006 旧 data reference-only guardrail

CR-006 后，旧 repo `data/` 只保留为人工 reference-only 线索：它可以帮助用户在另行授权后手动理解历史口径，但不能作为 fallback、迁移源、覆盖证明、测试 fixture、smoke 前置条件或运行时输入。任何 agent、测试、reader、comparison、Backtrader optional backend、Notebook 或数据层 job 默认都不得读取、列出、复制、迁移、比对、删除旧 repo `data/**`。
...
| `process/VALIDATION-ENV.yaml` 元数据滞后 | ADVISORY | 历史 `story_id` 仍指向早期 Story，但 `approval.confirmed=true`，当前范围以 `STATE`、`STORY-STATUS` 和验证报告为准。 |
