...
运行 ML 训练、walk-forward 全流程或 `tests/experiments/test_ml_factor_strategy_suite_exp23_29.py` 前，先安装并启用该组：
...
| 编号 | 入口 | 责任 |
|---:|---|---|
| 06/07 | `experiments/run_momentum_rsi_backtest_exp06_07.py` | 动量与 RSI 策略本地回测报告。 |
| 08 | `experiments/run_macd_backtest_exp08.py` | MACD 金叉 / 死叉策略回测报告。 |
| 09 | `experiments/run_strategy_parameter_sensitivity_exp09.py` | 多策略参数敏感性分析。 |
| 10 | `experiments/run_out_of_sample_overfit_risk_exp10.py` | 样本外测试与过拟合风险排序。 |
| 12 | `experiments/run_market_regime_segments_exp12.py` | 市场环境分段策略比较。 |
| 13 | `experiments/run_strategy_comparison_exp13.py` | 代理 benchmark / 真实 benchmark 与交易成本对比。 |
| 14 | `experiments/run_data_benchmark_audit_exp14.py` | 数据与 benchmark 审计报告。 |
| 15 | `experiments/run_factor_framework_exp15.py` | 因子框架与单因子回测。 |
| 16 | `experiments/run_momentum_factor_validation_exp16.py` | 动量因子验证。 |
| 17-21 | `experiments/run_multifactor_strategy_suite_exp17_21.py` | 多因子套件与研究报告。 |
| 23-29 | `experiments/run_ml_factor_strategy_suite_exp23_29.py` | ML 因子套件；需要 `ml` dependency group。 |
| 30-36 | `experiments/run_risk_tradability_integration_exp30_36.py` | Stage 5 多因子研究与候选产出。 |
| turnover | `experiments/run_low_turnover_double_sort.py` | abnormal turnover 因子研究；CR140 Phase 3 仅做合成 fixture 重构等价验证。 |
