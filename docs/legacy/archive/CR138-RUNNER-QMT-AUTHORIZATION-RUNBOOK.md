# CR138 Runner / QMT Gateway Authorization Runbook 归档入口

> Canonical non-CR entry: [../reference/RUNNER-QMT-AUTHORIZATION.md](../reference/RUNNER-QMT-AUTHORIZATION.md).
> 本文件只作为历史审计归档；新用户应优先阅读 canonical reference。

## Current Boundary

CR138 默认不授权真实 runtime、QMT、MiniQMT、XtQuant、gateway 启动、端口绑定、凭据读取、账户 / 行情 / 订单真实查询、submit/cancel、simulation/live、NAS、provider/lake/catalog 或 Git remote 写入。只有用户为单次运行提供明确 runtime authorization 时，才允许在该时间窗口内执行授权清单中的命令。
