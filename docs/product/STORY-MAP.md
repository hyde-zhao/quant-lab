...
| v1.6 | 2026-07-15 | meta-pm | 增量追加 CR171 四个 decision-oriented outcomes；CP8 是 decision-only closure，不创建正式 Story、DAG、Wave、LLD 或文件所有权。 |
| v1.7 | 2026-07-16 | meta-pm（pm-wu） | 增量追加 CR172 四个 activation product outcomes；PATH-C 仅作为 CP2 推荐，不创建正式 Story、DAG、Wave、LLD 或文件所有权。 |
| v1.8 | 2026-07-16 | meta-pm（pm-zheng） | 增量追加 CR173 四个 offline-methodology product outcomes；仅作 CP2/CP3 输入，正式 Story、DAG、Wave、LLD 和文件所有权均为 0。 |
| v1.9 | 2026-07-16 | meta-pm（pm-zheng） | 按 CP3 `PASS_BY_SPLIT` 将 CR173-O04 从 C1 projection 改为 standalone evidence public-boundary stop；四个 outcome ID 不变，public projection 不进入 CR173 的 Story/Feature/实现切片。 |
| v2.0 | 2026-07-16 | host-orchestrator | 补齐 CR172/CR173 关联追溯；当前仍为 CP3 待审的四个 estimator-only outcome，不创建 Story、DAG、Wave 或文件所有权。 |
| v2.1 | 2026-07-17 | meta-pm（pm-wu） | CR172 scope-delta 前轮草案追加 PATH-I outcome；部署与授权表述已由 v2.2 correction R1 全量替换；保留原四个 activation outcome 和 PATH-B 历史，不创建 Story/DAG/Wave。 |
| v2.2 | 2026-07-17 | meta-pm（pm-wu） | correction R1 将 O06~O08 改为 research-local canonical/NAS replica/execution cache 与六类动作，并新增 O10：执行机本地信号、低频 mailbox、intraday 独立 CR。 |
| v2.3 | 2026-07-17 | meta-pm（pm-wu） | correction R2 将 O09/O10 收缩为 `FU-CR173-001` 条件前置、SignalBatch 精确 8 字段边界和两个 deferred 信号候选，并新增阶段守卫 outcome；不创建 Story/DAG/Wave。 |
| v2.4 | 2026-07-17 | meta-pm（pm-wu） | 回填 CR172 PATH-I scope-delta CP2 用户批准；outcome 内容不变，只把产品规划状态推进为 confirmed-CP2，并明确当前仅解锁 CP3、正式 Story/DAG/Wave 仍为 0。 |

## 状态

- 文档状态：confirmed-CP2（CR172 PATH-I scope delta）
- 关联 CR：`CR-157` / `CR-158` / `CR-163` / `CR-164` / `CR-166` / `CR-168` / `CR-169` / `CR-170` / `CR-171` / `CR-172` / `CR-173`
- 当前门禁：CR172 PATH-I scope-delta CP2 已于 `2026-07-17T16:54:09+08:00` 获用户批准，当前只解锁 CP3 design-only；以下 outcomes 仍不是 Story ID，不写入 `DEVELOPMENT-PLAN.yaml`，正式 Story/DAG/Wave 数仍为 0。
...
| CR168-O04 | 保守接入联合 Gate 4 | 1 条 C3 compatibility projection；C4 absent-no-na-reason；adapter 以 8-key denylist、strict allowlist、前置拒绝与后置非 PASS 断言局部阻断 reason 逃逸；canonical 全局硬化不在本 CR | P0 | CP2 后进入 CP3 |
...
这些 outcome 是 CR171 已关闭的决策证据；任何后续 activation 的正式 Story 仍须由独立 CR 经 CP2/CP3 创建。

## CR172 Product-planning Outcomes（非正式 Story）

| Outcome ID | 用户结果 | 可验证成果 | 优先级 | Gate |
|---|---|---|---|---|
| CR172-O01 | 以最小 blast radius 启动 activation | 5/5 有限字段候选与 PATH-C C1-first 默认；PATH-A/B 触发条件显式 | P0 | CP2 |
| CR172-O02 | 在 estimator 未完成时仍可安全设计 C1 通道 | effective count typed-unavailable、raw alias=0、C1 computable/admission=false | P0 | CP2→CP3 |
| CR172-O03 | 让后续 C2/C3 有独立治理和回滚 | PATH-C 后 C2/C3 默认两个独立 runtime-high-risk CR；顺序 slice 仅作 CP2 备选 | P0 | CP2 |
| CR172-O04 | 防止 owner 合并和局部成功扩大权限/声明 | 同 revision/hash 双 ledger、风险交集、producer-local fail-closed、aggregate OR-pass=0 | P0 | CP2→CP3 |
| CR172-O05 | 先获得可信 trial-return 上游合同 | PATH-I per-trial portfolio-return object、family/run/trial identity、错误 returns 对象 fail-closed | P0 | CP2 |
| CR172-O06 | 让 artifact 跨主机稳定追踪且不依赖共享盘 | logical URI + hash、research-local active canonical、NAS verified replica、execution immutable cache | P0 | CP2→CP3 |
| CR172-O07 | 让每个真实数据动作可独立批准与撤销 | lake read/runtime+workspace/generation/R compute/NAS sync/execution pull `6/6` 分离 | P0 | CP2→future runtime gates |
| CR172-O08 | 隔离研究、副本、代码与执行数据面 | 四组件 `4/4` ownership、GitHub ceiling、execution direct-NAS runtime read=0 | P0 | CP2→CP3 |
| CR172-O09 | 替换新运行路径并保持 R 声明真实 | 新 run 语义路径、legacy 只读、declared-exact/empirical 分类；DQ-003 降级可继续设计，positive empirical claim 以 `FU-CR173-001` 为硬前置 | P0 | CP2→CP3/future candidate |
| CR172-O10 | 冻结最小信号边界而不预做 exchange | 默认执行机本地生成；低频/EOD 仅精确 8 字段 immutable batch contract；详细 exchange 与 intraday 分别进入两个 DF 候选 | P0 | CP2→CP3/backlog |
| CR172-O11 | 让每个阶段只能作出与证据相符的声明 | CP2 only-unlock-CP3、CP3 design-only、CP5 repository-local、CP7 real actions 0/6、CP8 max `path_i_design_ready=true` 且五项授权/就绪 flag=false | P0 | CP2→CP8 |

这些 outcome 已作为 scope-delta CP2 批准基线和 CP3 产品规划输入。正式 Story 数、文件 owner、DAG 和 Wave 必须在 CP3 通过后由 meta-se 决定；当前不得预建 instrumentation、C1/C2/C3 Story。PATH-B/CR173 方法学历史继续有效；即使 PATH-I 产品合同、repository-local fixture 或未来验证完成，也不得自动恢复 PATH-C/A。未来 PATH-C/A CP2 还必须对 empirical path 显式选择完成 `FU-CR173-001`、拆 future activation 或 DQ-003 typed-unavailable 降级；详细信号能力只保留 `DF-CR172-SIGNAL-BATCH-EXCHANGE` 与 `DF-CR172-INTRADAY-REALTIME-SIGNAL` 候选。

## CR173 Product-planning Outcomes（非正式 Story）

| Outcome ID | 用户结果 | 可验证成果 | 优先级 | Gate |
|---|---|---|---|---|
| CR173-O01 | 获得不会把 raw count 冒充相关结构有效维度的产品合同 | participation-ratio 二阶 estimand、non-alias、失败语义与 Li–Ji/BH/FWER/DSR calibration claim=`0` | P0 | CP2→CP3 |
| CR173-O02 | 获得可审计、可恢复的 typed evidence | 七字段 `7/7`、provenance、typed-unavailable/blocked 与新版本恢复 | P0 | CP3→CP5 |
| CR173-O03 | 证明离线方法稳定且策略无关 | 六类 vectors `6/6` × `3/3` repeats、strategy identity 与真实输入=`0` | P0 | CP5→CP7 |
| CR173-O04 | 交付独立证据并在 public C1 边界停止 | standalone evidence `1/1`；public C1 projection/write=`0/0`；C1/Gate1/admission 保持 unavailable；CR172 auto-resume=`0` | P0 | CP5→CP8 |

以上仅是用户 outcome，不是正式 Story。CP3 已将候选方法收敛为 participation-ratio estimator-only，但人工批准前 Story、DAG、Wave、LLD、文件 owner 数均为 `0`。public C1 versioned projection 只是 backlog 后续候选，未创建正式 CR，也不得回填到 CR173 的 Story/Feature。
