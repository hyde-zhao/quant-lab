...
| 版本 | 日期 | 修订人 | 变更要点 |
|---|---|---|---|
| v1.0 | 2026-07-11 | meta-doc | 增量补充 CR-163 trial lineage CLI 操作、可用性解释与故障恢复限制。 |
...
默认数据湖和研究产物放在本机目录，项目通过 `MARKET_DATA_LAKE_ROOT` 或命令行 `--lake-root` 读取数据湖；推荐本机数据湖根目录写为 `<local-quant-lab-data-lake-root>`，并与 NAS 末级目录 `/data-lake` 保持一致；研究数据根目录写为 `<local-quant-lab-research-root>`。NAS 不作为默认实时读写路径，只作为 rsync daemon 同步目标。
...
### 5.3 为未来研究运行启用 Trial Lineage

Mature multifactor research 的稳定入口是 `scripts/research/run_multifactor_strategy_research.py`。需要 instrumented trial lineage 时，必须在原有研究参数之外成对提供：

```text
--lineage-spec <path> --lineage-root <path>
```

不要只提供其中一个参数。CLI 和 admission 的结果按下表解释：

| 输入 / family 状态 | 结果 | 操作要求 |
|---|---|---|
| 两个参数都未提供 | `typed_unavailable` | 可以按兼容路径运行，但不能声称已有 trial lineage。 |
| 仅提供一个参数 | `blocked` | 补齐参数并以有效 spec 重新开始；不得把不完整输入降级为 unavailable。 |
| lineage spec 无效 | `blocked` | 修正 spec 后重新开始；不得继续写入或投影 `present`。 |
| 参数有效，但 family 未 seal 或 validation 不完整 | 非 `present` | 完成 seal 和完整 validation；目录或局部记录存在不等于 lineage 可用。 |
| family 已 seal 且完整 validation 通过 | `present` | 仅适用于未来 instrumented run；以验证后的 family 事实投影准入字段。 |

`raw_trial_count` 是系统从已验证 family lineage 得出的事实，不能由研究报告手工填写或覆盖。CR-163 不计算统计相关性校正后的 `effective_trial_count`，该字段仍为 `typed_unavailable`，所以 C1 不可计算；不得用 raw count 冒充 effective count。

如果记录过程中 crash，或检查发现 malformed tail，不要 resume、截断尾部后继续写、或给原 family 补 seal。保留失败 family 的证据，创建新的 family identity，再从头执行。精确 10,000 trial 只说明 characterization 覆盖，不是性能、容量或恢复 SLA。

CR155 的历史候选不做 reconstruction 或 backfill，继续保持 admission `blocked`、`paper_candidate=false`。本文档所述 lineage 能力本身不授权真实研究运行、数据访问、凭据读取、provider / NAS / broker 操作、simulation、paper、live、publish 或 deployment。
...
uv run --python 3.11 python scripts/qmt/build_multifactor_runtime_inputs.py \
  ...
  --output-dir <local-qmt-runtime-output-dir>
```
...
| lineage 参数只提供一个，或 spec 无效 | 结果必须保持 `blocked`；修正输入并以有效 family 重新开始，不降级为 `typed_unavailable`。 |
| trial family crash 或尾部 malformed | 不 resume、不截断修复、不补 seal；保留失败证据并创建新的 family identity。 |
