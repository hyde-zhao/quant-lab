...
Agent 协作边界：当前会话的 Host Orchestrator 主进程负责编排 CR、检查点和人工门禁；功能 Agent 只在对应阶段按需参与。上述角色不越权修改代码、测试、真实数据、报告数据或安装脚本，也不得读取 `.env`、token、账号或任何凭据。
...
| 依赖环境 | `uv` | 是 | 使用 `uv sync --python 3.11` 和 `uv run --python 3.11 ...`；package identity 可用 `uv run quant_lab --help` 或 `uv run python -m quant_lab.cli --help` 检查。 |
...
项目代码只读取环境变量；`pyproject.toml` / `uv.lock` 已落地 `tushare==1.4.29` 和 `jqdatasdk` dependency group。`.env` 文件只能由用户本机维护；Agent 不得读取、打印或代为检查 `.env`、token、账号或任何凭据。Tushare 正式命令入口通过 `uv run --env-file .env --group tushare --python 3.11 ...` 加载本机 `.env` 并启用该 group，该类命令仅供用户在明确授权的本地环境中手工执行。一种方式是让 `uv run` 读取该文件：
...
如果任何人要求直接启动 simulation/live、打开 QMT / MiniQMT / GUI、调用 broker API、发单、撤单、查询账户、读取凭据、拉取真实 snapshot、写真实 broker lake、provider fetch、写真实 lake 或 publish，应停止当前流程，回到 Host Orchestrator 发起的 CR / CP / human-gate 授权路径；未获明确授权时默认拒绝执行。
...
- CR-012 limited-window readiness audit 口径修正已完成代码、测试和文档增量；CR-013 已完成 `S01` 至 `S04` 的 CP7 验证与文档收敛，当前等待 Host Orchestrator 创建 CP8 终验。
...
| CR-013 CP8 终验 | PENDING | CR013-S01..S04 已 CP7 PASS；文档收敛复核无 BLOCKING / REQUIRED 缺口，等待 Host Orchestrator 创建 CP8 自动预检与人工终验稿。 |
