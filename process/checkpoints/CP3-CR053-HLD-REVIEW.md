...
|---|---|---|---|---|---|---|---|
| DQ-CP3-CR053-01 | architecture | 是否采用逻辑 NAS root 映射，而不扫描 NAS？ | 是，按 `QUANT_LAB_*_ROOT` 映射 512G hot、4T warm、14T cold。 | A: 立即扫描 NAS；B: 只做 Git-only。 | 推荐方案满足不扫描前提且能给出目录方案；A 需授权；B 不满足用户要求。 | 真实路径后续可能不同。 | CR058/CR060 前用户给出真实路径或授权 read-only inventory。 |
...
| DQ-CP3-CR053-05 | security | 交易主机是否只读消费 package exchange？ | 是，不挂载 full research archive。 | A: 交易主机挂 full archive；B: 交易主机保留完整研究仓库。 | 推荐方案最小权限；备选扩大交易主机风险面。 | 后续 package import 需要 checksum。 | 隔离测试机可临时 read-only checkout。 |

### 决策摘要

| 字段 | 内容 |
|---|---|
| 推荐决策 | 回复 `approve`，接受 DQ-CP3-CR053-01..05 推荐方案，并进入后续 CP4 / CP5 设计证据阶段。 |
...
| 用户需决策事项 | 是否接受 DQ-CP3-CR053-01..05：逻辑 NAS root 映射、manifest-first 两阶段传输、4T RAID warm archive + 14T cold backup、真实迁移延后到 CR058 CP6、交易主机只读消费 package exchange。 |
...
| 条目 | 状态 | 证据 | 审查意见 |
|---|---|---|---|
| CP2 approved | 待审查 | `process/checkpoints/CP2-CR053-REQUIREMENTS-BASELINE.md` |  |
| HLD 已生成 | 待审查 | HLD |  |
| ADR 已生成 | 待审查 | ADR |  |
| CP3 自动预检 PASS | 待审查 | `process/checks/CP3-CR053-HLD-CONSISTENCY.md` |  |

## Checklist

| # | 检查项 | 审查结果 | 证据 | 审查意见 |
|---|---|---|---|---|
| 1 | NAS 逻辑目录映射可接受 | 待审查 | DQ-CP3-CR053-01 |  |
| 2 | 数据传输协议可接受 | 待审查 | DQ-CP3-CR053-02 |  |
| 3 | 备份方案可接受 | 待审查 | DQ-CP3-CR053-03 |  |
| 4 | 真实迁移后置到 CR058 可接受 | 待审查 | DQ-CP3-CR053-04 |  |
| 5 | 交易主机边界可接受 | 待审查 | DQ-CP3-CR053-05 |  |
| 6 | 不授权边界清晰 | 待审查 | 本文件不授权项 |  |
...
| 无阻断项 | 待审查 | CP3 自动预检 | PASS。 |
| approve 后可进入 CP4 | 待审查 | HLD / ADR | 不等于授权真实迁移。 |

## Deliverables

| 交付物 | 路径 | 审查结果 | 审查意见 |
|---|---|---|---|
| HLD | `docs/design/HLD-CR053-QUANT-LAB-MIGRATION-INVENTORY-AND-DRY-RUN.md` | 待审查 |  |
| ADR | `docs/design/ARCHITECTURE-DECISION-CR053.md` | 待审查 |  |
| CP3 Context | `process/context/CP3-CR053-DESIGN-CONTEXT.yaml` | 待审查 |  |
| CP3 自动预检 | `process/checks/CP3-CR053-HLD-CONSISTENCY.md` | 待审查 |  |
