...
| `process/changes/CR-019-FOLLOW-UP-TRACKING-2026-05-31.md` | 更新原文档 | 把 CR-030 从 candidate 转为 active formal CR。 | approved-by-this-intake |
| `process/changes/CR-INDEX.yaml` | 更新原文档 | 保持 active formal CR 与后续候选一致性。 | approved-by-this-intake |
| `process/STATE.md` | 更新原文档 | 恢复点必须指向 CR-030 active-cp2-intake。 | approved-by-this-intake |
...
CR-030 可以启动为正式 active CR，当前阶段为 `active-cp2-intake`。它应作为多因子研究闭环主线，优先解决研究对象模型、因子评价、组合构建、实验追溯和研究到执行准入，而不是直接推进 QMT 或外部框架集成。
