...
| 5 | 通用因子计算从第三章模块迁出 | PASS | `engine/factor_calculators.py` |
| 6 | 通用统计检验从第三章模块迁出 | PASS | `engine/factor_statistics.py` |
| 7 | 第三章模块保留为适配层 | PASS | `engine/chapter3_factor_replication.py` 调用 `compute_equity_factor_matrices(...)` |
| 8 | 第三章兼容入口保留 | PASS | `chapter3_factor_definitions()`、`factor_matrices_to_panel(...)` wrapper |
| 9 | 过程文档更新 | PASS | `process/research/chapter3_factor_replication/README.md` |
| 10 | 新增测试覆盖分层边界 | PASS | `tests/test_factor_library.py`、`tests/test_factor_calculators.py`、`tests/test_factor_statistics.py` |
...
| `engine/factor_calculators.py` | 新增 | 七个通用因子矩阵计算、方向处理、缩尾、zscore、面板输出。 |
...
| `tests/test_factor_calculators.py` | 新增 | 验证七个因子矩阵和缺输入 limitations。 |
