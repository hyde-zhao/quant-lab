...
| 4 | 七个因子矩阵计算可用 | PASS | `tests/test_factor_calculators.py` |
| 5 | 缺输入 fail-closed limitations 可见 | PASS | `tests/test_factor_calculators.py` |
| 6 | 排序和统计工具可复用 | PASS | `tests/test_factor_statistics.py` |
| 7 | 第三章 policy 行为未回退 | PASS | `tests/test_chapter3_factor_replication.py` |
| 8 | CR030 关键合同未回退 | PASS | CR030 回归测试 |
| 9 | 编译通过 | PASS | `py_compile` |
| 10 | diff 空白检查通过 | PASS | `git diff --check HEAD~1..HEAD` |
