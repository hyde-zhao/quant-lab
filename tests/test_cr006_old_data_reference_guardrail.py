...
def test_guardrail_allowlist_and_post_s03_targets_are_explicit() -> None:
    ...
    assert "tests/test_cr006_old_data_reference_guardrail.py" in allowed
    assert "engine/backtrader_adapter.py" in POST_S03_STATIC_SCAN_TARGETS
    assert "tests/test_cr006_backtrader_clean_feed.py" in POST_S03_STATIC_SCAN_TARGETS
...
def test_static_scan_reports_no_high_risk_old_data_or_secret_claims() -> None:
    ...
    for relative_path in ACTIVE_STATIC_SCAN_TARGETS:
        ...
        if relative_path == "tests/test_cr006_old_data_reference_guardrail.py":
            continue
    ...
    assert "tests/test_cr006_old_data_reference_guardrail.py" in scanned
    assert violations == []
