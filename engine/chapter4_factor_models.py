...
def panel_to_factor_matrices(
    ...
    *,
    ...
    for factor_id in factor_ids:
        ...
        matrix = part.pivot_table(index="trade_date", columns="symbol", values=value_column, aggfunc="last")
        matrix = matrix.sort_index().apply(pd.to_numeric, errors="coerce")
...
def labels_to_forward_return_matrix(labels: pd.DataFrame) -> pd.DataFrame:
    work = labels.copy()
    work["trade_date"] = work["trade_date"].astype(str)
    matrix = work.pivot_table(index="trade_date", columns="symbol", values="forward_return", aggfunc="last")
