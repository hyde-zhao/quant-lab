<!-- myflow:managed:begin v=1 commit=05cbfdc generated=2026-05-23T11:49:47Z -->
<!-- myflow-managed: version=1.0.0 canonical-commit=05cbfdc generated=2026-05-23T11:49:47Z -->
...
| Agent | 提示词文件 | 职责 | 唤醒条件 |
|-------|-----------|------|---------|
| **meta-pm** | `agents/meta-pm.md` | 快速调研（阶段零）+ 头脑风暴式场景发现（USE-CASES.md，含画像/指标/候选理解与 trade-off）+ 需求结构化（REQUIREMENTS.md，含风险/里程碑）+ CP2 Decision Brief 输入 | 新请求进入、需求模糊、需求变更后重整 |
| **meta-se** | `agents/meta-se.md` | HLD 设计（含候选方案对比 + 5 层架构图 + 技术选型理由）+ CP3 多角色讨论输入 + Story 拆解（含文件布局 + TASK-ID 任务清单）+ 开发计划（含完成准则） | REQUIREMENTS.md 已确认（solution-design 和 story-planning 两阶段均由 meta-se 执行） |
| ~~**meta-dm**~~ | ~~`agents/meta-dm.md`~~ | ~~Story 拆解与并行计划~~ | ⚠️ **已废弃**，职责合并至 meta-se |
| **meta-dev** | `agents/meta-dev.md` | Story LLD 输出与批量确认闭环 + Agent/Skill 文件实现 + TASK-ID 增量追踪 + CP7 失败回修 | 存在已批准且可执行的 Story |
